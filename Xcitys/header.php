<header class="justify-content-center page-scroll" style="font-family: Poppins-Regular;">       
        <div class="topnav navbar-dark bg-dark nav-pills" id="myTopnav">
            <a href="#home" class="active" id="home"><img src="logo.png" style="width:15px;height:20px;"/>  Xcity</a>
            <script type="text/javascript">
                document.getElementById("home").onclick = function () {
                    location.href = "index.php";
                };
              </script>
            <a href="sante.php">Sante</a>
            <a href="environnement.php">Environnement</a>
            <a href="logement.php">Logement</a>
            <a id="Connexion" style='color: white;'>Connexion</a>
              <script type="text/javascript">
                document.getElementById("Connexion").onclick = function () {
                    location.href = "indexx.php";
                };
              </script>
            <a href="javascript:void(0);" class="icon" onclick="myFunction()">
                <i class="fa fa-bars"></i>
            </a>                        
        </div>
    </header>   